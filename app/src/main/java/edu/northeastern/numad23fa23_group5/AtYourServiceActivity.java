...
//            url = new URL("https://jsonplaceholder.typicode.com/posts?_delay=5000");
            ...
            if (responseCode == HttpURLConnection.HTTP_OK) {
                ...
                handleSuccessfulResponse(urlConnection);
            } else {
                // Handle HTTP error response
                handleErrorResponse(urlConnection, responseCode);
            }
        } catch (Exception e) {
            // Handle exceptions
            handleException(e);
    ...
    private void handleSuccessfulResponse(HttpURLConnection connection) {
        try {
    ...
    private void handleErrorResponse(HttpURLConnection connection, int responseCode) {
        try {
            ...
            handler.post(new Runnable() {
                ...
                public void run() {
                    View rootView = findViewById(android.R.id.content);
                    Snackbar.make(rootView, "HTTP Error: " + responseCode, Snackbar.LENGTH_SHORT).show();
                    progressBar.setVisibility(View.GONE);
                }
            });
        } catch (IOException e) {
